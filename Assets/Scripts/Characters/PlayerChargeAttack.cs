...
public class PlayerChargeAttack : MonoBehaviour
{
    ...
    public ParticleSystem chargeAttackTrail;
    ...
    void Start()
    {
        ...
        if (chargeAttackTrail.isPlaying) chargeAttackTrail.Stop(true, ParticleSystemStopBehavior.StopEmitting);
    }
    ...
    IEnumerator PerformChargeAttack()
    {
        ...
        // 3. 시간 정지하고 발동준비
        if (!chargeAttackTrail.isPlaying)
        {
        ...
        // 5. 원래 위치로 돌아오기 + 데미지주기
        transform.position = originalPosition; // 최종 위치 보정
